...
function bindInput() {
  ...
  const move = (cx, cy) => {
    if (!dragOn) return;
    cam.yaw += (cx - dragX) * 0.0095;
    cam.pitch = clamp(cam.pitch - (cy - dragY) * 0.0095, 0.06, 1.46);
...
function drawReactor(time) {
  ...
  if (!reduceMotion && !dragOn && time - lastInteract > 3500) cam.yaw += dt * 0.00006;
  ...
  // toroidal field coils (poloidal rings caging the plasma)
  const NC = 16;
  for (let ci = 0; ci < NC; ci += 1) {
    const th = (ci / NC) * Math.PI * 2;
    const loop = [];
    for (let a = 0; a <= 40; a += 1) loop.push(torusPt(th, (a / 40) * Math.PI * 2, A_COIL));
    const pr = projectAll(loop);
    const b = depthBright(pr.depth);
    ...
    prims.push({ z: pr.depth, d: () => {
      ...
      if (b > 0.55) { ctx.shadowColor = "rgba(168,150,255,0.55)"; ctx.shadowBlur = 6 * b; }
  ...
  for (let di = 0; di < ND; di += 1) {
    ...
    prims.push({ z: c.depth - 0.0005, d: () => {
      ...
      const g = ctx.createRadialGradient(c.x, c.y, 0, c.x, c.y, rad * 1.18);
      g.addColorStop(0, `rgba(${pColor[0]},${pColor[1]},${pColor[2]},${0.16 + intensity * 0.24})`);
      g.addColorStop(0.45, `rgba(${pColor[0]},${pColor[1]},${pColor[2]},${0.06 + intensity * 0.10})`);
      g.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = g;
      ctx.beginPath(); ctx.arc(c.x, c.y, rad * 1.18, 0, Math.PI * 2); ctx.fill();
      ctx.restore();
    } });
  }
  ...
  for (let i = 0; i < prims.length; i += 1) prims[i].d();
...
function drawTitle(ctx, w, h) {
  ...
  ctx.fillText("DRAG TO ORBIT · SCROLL TO ZOOM", Math.max(16, w * 0.05), Math.max(40, h * 0.06) + 16);
  ctx.restore();
}
